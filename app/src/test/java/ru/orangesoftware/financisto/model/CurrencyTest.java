...
public class CurrencyTest extends AbstractDbTest {
    ...
    public void should_format_amount_according_to_the_selected_currency() {
        ...
        currency.setSymbol("$");
        String actualString = Utils.amountToString(currency, 100000);
    ...
    @Test public void should_format_symbol_according_to_the_selected_format() {
        ...
        currency.setSymbol("$");
        //when
        currency.setSymbolFormat(SymbolFormat.RS);
        assertEquals("1,000.0 $", Utils.amountToString(currency, 100000));
        assertEquals("+1,000.0 $", Utils.amountToString(currency, 100000, true));
        assertEquals("-1,000.0 $", Utils.amountToString(currency, -100000));
        //when
        currency.setSymbolFormat(SymbolFormat.R);
        assertEquals("1,000.0$", Utils.amountToString(currency, 100000));
        assertEquals("+1,000.0$", Utils.amountToString(currency, 100000, true));
        assertEquals("-1,000.0$", Utils.amountToString(currency, -100000));
        //when
        currency.setSymbolFormat(SymbolFormat.LS);
        assertEquals("$ 1,000.0", Utils.amountToString(currency, 100000));
        assertEquals("+$ 1,000.0", Utils.amountToString(currency, 100000, true));
        assertEquals("-$ 1,000.0", Utils.amountToString(currency, -100000));
        //when
        currency.setSymbolFormat(SymbolFormat.L);
        assertEquals("$1,000.0", Utils.amountToString(currency, 100000));
        assertEquals("+$1,000.0", Utils.amountToString(currency, 100000, true));
        assertEquals("-$1,000.0", Utils.amountToString(currency, -100000));
    }
    ...
    @Test public void should_reset_default_flag() {
        ...
        assertTrue(db.get(Currency.class, c1.getId()).isDefault());
        Currency c2 = CurrencyBuilder.withDb(db).name("SGD").title("Singapore Dollar").symbol("S$").makeDefault().create();
        //There can be only one!
        assertFalse(db.get(Currency.class, c1.getId()).isDefault());
        assertTrue(db.get(Currency.class, c2.getId()).isDefault());
    }
    ...
    @Test public void should_return_home_currency() {
        ...
        assertEquals("SGD", db.getHomeCurrency().getName());
    }
    ...
    @Test public void should_return_empty_currency_if_home_is_not_set() {
        ...
        assertEquals("", db.getHomeCurrency().getName());
    }
    ...
    @Test public void should_set_home_currency_if_it_has_not_been_set_and_the_same_currency_is_used_in_all_accounts() {
        ...
        assertEquals(Currency.Companion.getEMPTY(), db.getHomeCurrency());
